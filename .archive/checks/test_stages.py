...
def make_run(path, workloads=None, limit=2):
    ...
    server = {'identity': serving_identity(config), 'status': 'ready',
              'environment': {'gpu_name': 'A100 fixture', 'packages': {}}}
    server['serving_fingerprint'] = digest(server['identity'])
    identity = {'config': config, 'sources': sources, 'server': server['serving_fingerprint'],
                'requests': [[r['workload'], r['prompt_id'], r['request_sha256'], r['source_sha256']] for r in records]}
    write_json(path / 'manifest.json', {'status': 'complete', 'server_provenance': server, 'config': config, 'sources': sources,
...
def fake_workers(monkeypatch):
    ...
    monkeypatch.setattr('driftbench_runner.credentials.load_credentials', lambda: None)
    return calls
...
def test_script_dry_run_has_no_credentials_gpu_or_output(tmp_path):
    ...
    result = subprocess.run(['bash', str(ROOT / 'scripts/results.sh'), 'infer', '--config',
                             str(ROOT / 'suites/a100.json'), '--run-id', 'a100-r01',
...
def test_infer_dispatch_defers_all_evaluation_and_records_stages(tmp_path, monkeypatch):
    ...
    loaded = []
    monkeypatch.setattr('driftbench_runner.credentials.load_credentials', lambda: loaded.append(True))
    ...
    args = SimpleNamespace(command='infer', config=str(ROOT / 'suites/a100.json'),
